...
export const registerWithCredential = async (data: IRegisterSchema) => {
  try {
    ...
    const token = generateToken({
      user: { ...validationResult.data, password: hashedPassword },
    })
...
export const forgotPasswordWithCredentials = async (
  ...
  try {
    ...
    if (user?.provider !== 'credentials') {
      return {
        error: `This account is signed with ${user?.provider}, you can't use this function`,
      }
    }
